...
useEffect(() => {
    if (driverData && driverData._id) {
        const url = `http://localhost:5000/busdriveraccount/${driverData._id}`;
        fetch(url)
            .then((res) => {
                if (!res.ok) {
...
const onSubmit = (e) => {
    ...
    fetch(`http://localhost:5000/busdriveraccount/${driverData?._id}`, {
        method: "PUT",
        ...
        .then(res => res.json())
        .then(() => {
            return fetch(`http://localhost:5000/busdriveraccount/${driverData?._id}`);
        })
        .then(res => res.json())
        .then((updatedData) => {
